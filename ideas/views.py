...
def ajax_new_center_square_idea(request,
    template_name='ideas/partials/center_square_idea.html'):
    if not request.GET.get('content', None):
        return JsonResponse({
            'errors': True,
    ...
    return JsonResponse({
        'success': True,
        'content': render_to_string(template_name, {'idea': idea})
    })


@login_required
def ajax_new_square_idea(request,
    template_name='ideas/partials/square_idea.html'):
    if not request.GET.get('content', None):
        return JsonResponse({
            'errors': True,
            'error_message': 'Unable to create a blank center square idea'
...
def ajax_new_center_square_vote(request):
    try:
        idea = CenterSquareIdea.objects.get(pk=request.GET.get('idea', None))
    ...
    if idea.votes.filter(user=request.user).exists():
        ...
        if ( request.GET.get('vote_type', None) == 'positive' and \
            vote.is_positive() ) or \
            ( request.GET.get('vote_type', None) == 'negative' and \
            vote.is_negative() ):
            return JsonResponse({
                'errors': True,
                'error_message': 'Your vote has already been cast'
            })
        elif request.GET.get('vote_type', None) in ('positive', 'negative'):
    ...
    if request.GET.get('vote_type', None) == 'positive':
    ...
    elif request.GET.get('vote_type', None) == 'negative':
...
def ajax_new_square_vote(request):
    try:
        idea = SquareIdea.objects.get(pk=request.GET.get('idea', None))
    except CenterSquareIdea.DoesNotExist:
        return JsonResponse({
            'errors': True,
            'error_message': 'Unable to find idea'
